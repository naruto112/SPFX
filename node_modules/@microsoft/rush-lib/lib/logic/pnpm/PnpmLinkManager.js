...
class PnpmLinkManager extends BaseLinkManager_1.BaseLinkManager {
     ...
     */
    _linkProject(project, rushLinkJson, pnpmShrinkwrapFile) {
        ...
        const folderNameInLocalInstallationRoot = uriEncode(node_core_library_1.Text.replaceAll(absolutePathToTgzFile, path.sep, '/')) +
            folderNameSuffix;
        // tslint:disable-next-line:max-line-length
        // e.g.: C:\wbt\common\temp\node_modules\.local\C%3A%2Fwbt%2Fcommon%2Ftemp%2Fprojects%2Fapi-documenter.tgz\node_modules
        const pathToLocalInstallation = path.join(this._rushConfiguration.commonTempFolder, RushConstants_1.RushConstants.nodeModulesFolderName, '.local', folderNameInLocalInstallationRoot, RushConstants_1.RushConstants.nodeModulesFolderName);
        ...
        if (!parentShrinkwrapEntry) {
            throw new node_core_library_1.InternalError('Cannot find shrinkwrap entry using dependency key for temp project: ' +
